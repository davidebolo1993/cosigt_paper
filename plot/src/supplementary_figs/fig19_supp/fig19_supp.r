...
if (length(args) < 2) {
  cat("Usage: Rscript fig19_supp.r <output_prefix> <hla_results>  \n")
...
ggsave(output_png, plot = hla_plot, 
       ...
       dpi = 600, limitsize = FALSE,units = "cm",create.dir=T)
...
ggsave(output_pdf, plot = hla_plot, 
       width = plot_width, height = plot_height, 
       limitsize = FALSE,units = "cm",create.dir=T)
...
ggsave(output_svg, plot = hla_plot, 
       width = plot_width, height = plot_height, 
       device = "svg", limitsize = FALSE,units = "cm",create.dir=T)
...
ggsave(output_eps, plot = hla_plot, 
       width = plot_width, height = plot_height, 
       device = "eps", limitsize = FALSE,units = "cm",create.dir=T)
